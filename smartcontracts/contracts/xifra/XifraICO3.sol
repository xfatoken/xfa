...
contract XifraICO3 {
    ...
    function setWhitelisted(address[] memory _whitelisted) external {
        require(msg.sender == xifraWallet, "OnlyXifra");

        for (uint256 i=0; i<_whitelisted.length; i++) {
            address _whitelist = _whitelisted[i];
            whitelisted[_whitelist] = true;
        }
    }

    function addTokensToUser(address _user, uint256 _amount) external {
        require(msg.sender == xifraWallet, "OnlyXifra");

        userBoughtTokens[_user] += _amount;
        icoTokensBought += _amount;
    }
    ...
    function buy(uint256 _paymentAmount, address _tokenPayment) external payable {
        ...
        if (msg.value == 0) {
            ...
            IERC20(_tokenPayment).safeTransferFrom(msg.sender, address(this), _paymentAmount);
    ...
    function withdrawBoughtTokens() external {
        ...
        if ((block.timestamp >= tokenListingDate) && (block.timestamp < tokenListingDate + 90 days)) {
            if ((boughtBalance <= _MIN_COINS_FOR_VESTING) || (whitelisted[msg.sender])) {
    ...
    function withdrawUSDT() external {
        ...
        if (usdtBalance > 0)
            IERC20(usdtToken).safeTransfer(xifraWallet, usdtBalance);
    }

    /**
     * @notice Withdraw USDCs
     */
    function withdrawUSDC() external {
        ...
        if (usdcBalance > 0)
            IERC20(usdcToken).safeTransfer(xifraWallet, usdcBalance);
    }
    ...
    /**
     * @notice If someone send tokens by mistake to the ICO smartcontract, we have the ability to recover those tokens
    ...
    function emegercyWithdraw(address _token) external {
        ...
        if (tokenBalance > 0)
            IERC20(_token).safeTransfer(xifraWallet, tokenBalance);
    }
